...
class TestTradeStudiesDirectory:
    ...
    @pytest.mark.parametrize(
        "filename",
        [
            "TS-28-10-TS01_geometry.yaml",
            "TS-28-10-TS02_distributed_architecture.yaml",
            "TS-28-10-TS03_insulation.yaml",
...
class TestTradeStudySchema:
    ...
    def test_has_ata_code(self, ts_data):
        assert ts_data.get("ata_code") == "28-10-00"
...
class TestArtifactCatalogTradeStudies:
    """ARTIFACT_CATALOG.yaml must include trade_studies entry."""
    ...
    def test_trade_studies_registered(self, catalog_data):
        ts = catalog_data.get("artifact_types", {}).get("trade_studies")
        assert ts is not None

    def test_trade_studies_count(self, catalog_data):
        ts = catalog_data["artifact_types"]["trade_studies"]
        assert ts["count"] == 3
