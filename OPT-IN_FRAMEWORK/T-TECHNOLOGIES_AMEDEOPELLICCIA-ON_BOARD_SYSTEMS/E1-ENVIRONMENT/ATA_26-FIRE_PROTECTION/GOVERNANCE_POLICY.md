# ATA 26 – Governance Policy

> Default baseline policy for ATA 26 Fire Protection configuration management.
...
## 4. Fire Protection Safety Escalation

Due to the safety-critical nature of fire protection systems:

| Trigger | Escalation Target | Timeout |
|---------|-------------------|---------|
| Any fire/smoke detection anomaly | STK_SAF | 24 hours |
| Suppression system failure | STK_SAF | 24 hours |
| Fire protection content generation | STK_SAF | 72 hours |
| H₂-related fire protection changes | STK_SAF | 72 hours |
| Undefined safety condition | STK_CM → CCB | HALT until resolved |

All fire protection anomalies **must** be escalated to STK_SAF within the specified timeout.
Failure to escalate is a **BREX Safety Violation** (SAFETY-002).

## 5. BREX Contract Requirement

All content generation, transformation, and publication for ATA 26 requires
an active BREX contract:

- **Contract:** `KITDM-CTR-LM-CSDB_ATA26`
- **Enforcement:** AUTHOR-002 (block without contract)

No ATA 26 content may be produced without a valid, ASIT-approved contract.
